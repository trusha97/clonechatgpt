...
export default function SearchButton() {
  ...
  return (
    <div
      ...
      className="view-transition-name-[var(--vt-composer-search-action)]"
    >
      <div>
        <span className="inline-block" data-state={isOpen ? "open" : "closed"}>
          <div
            className={`inline-flex h-9 rounded-full border text-[13px] font-medium border-gray-300 text-[#5D5D5D] hover:bg-gray-100 focus-visible:outline-black dark:focus-visible:outline-white transition-colors duration-200 ${
              isOpen ? "bg-black/10" : ""
            }`}
          >
            <button
              className="flex h-full min-w-8 items-center justify-center p-2 text-[#5D5D5D]"
            ...
            >
              ...
              <span className="ps-1 pe-1 whitespace-nowrap text-[#5D5D5D] [[data-collapse-labels]_&]:sr-only">
                Search
              </span>
            </button>
          </div>
